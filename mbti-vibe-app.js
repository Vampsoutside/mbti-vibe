    // ─── TMDB via Vercel proxy (bypasses India ISP blocks on api.themoviedb.org) ───
    const TMDB_API_KEY = "e0e3b455bdb46b3478dec7c9c9490c37";
    const TMDB_API = "/tmdb-api";
    const TMDB_IMG = "/tmdb-images";
    ...
    function hasValidPoster(poster) {
      ...
      if (p.startsWith("/tmdb-images/")) return true;
      return false;
    }
    ...
    function mapTmdbMovie(m) {
      ...
      if (isJunkTitle(m.title)) return null;
      if (Array.isArray(m.genre_ids) && m.genre_ids.length === 1 && m.genre_ids[0] === 99) return null;
      const poster = tmdbPoster(m.poster_path, "w342");
      if (!hasValidPoster(poster)) return null;
      return {
        Title: m.title,
        ...
        Poster: poster,
        Plot: m.overview || "",
        imdbRating: m.vote_average ? String(Number(m.vote_average).toFixed(1)) : null,
        Genre: "",
        Type: "movie",
        tmdbId: m.id,
      };
    ...
    async function tmdbFetch(path, params) {
      ...
      const url = TMDB_API + "/" + String(path).replace(/^\//, "") + "?" + qs.toString();
    ...
    async function tmdbByTitle(title) {
      const data = await tmdbFetch("search/movie", {
        query: title,
        include_adult: "false",
        language: "en-US",
      ...
      let best = results.find((r) => normalizeTitle(r.title) === want);
      if (!best) {
        best = results.find((r) => {
    ...
    async function tmdbDetails(tmdbId) {
      ...
      if (genres.toLowerCase() === "documentary") return null;
      return {
        Title: m.title,
        Year: (m.release_date || "").slice(0, 4) || "",
        imdbID: "tmdb-" + m.id,
    ...
    async function fetchMovies() {
      ...
      const cacheKey = "mv_cache_tmdb_v1_" + currentType + "_" + searchTermOffset;
      try {
      ...
      const exactHits = await Promise.all(
        seedSlice.map((title) => tmdbByTitle(title).catch(() => null))
      );
      ...
      if (candidates.length < 4 && queries.length) {
        const searchResults = await Promise.all(
          queries.map((q) => tmdbSearch(q).catch(() => []))
        );
      ...
      let movies = candidates.filter((m) => {
      ...
      for (const m of movies) {
        ...
        final.push(m);
        if (final.length >= MAX_RESULTS) break;
      }

      // Prefer liked titles first, then rating
      final.sort((a, b) => {
    ...
    function renderMovies(movies) {
      ...
      const safeMovies = (movies || []).filter(
        (m) => m && m.imdbID && m.Poster && m.Poster !== 'N/A' && ( /^https?:\/\//i.test(m.Poster) || String(m.Poster).startsWith('/tmdb-images/') )
      );
      ...
      list.innerHTML = safeMovies.map((m, idx) => {
        ...
        let posterUrl = '';
        if (rawPoster) {
          if (rawPoster.startsWith('/tmdb-images/')) {
            // Same-origin Vercel proxy — already sized (w342)
            posterUrl = rawPoster;
          } else if (/^https?:\/\//i.test(rawPoster)) {
            posterUrl = `https://images.weserv.nl/?url=${encodeURIComponent(rawPoster.replace(/^https?:\/\//i, ''))}&w=300&h=450&fit=cover&output=webp&q=80&default=1`;
          }
        }
